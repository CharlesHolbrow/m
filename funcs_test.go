...
func TestNoteGroup_Repeat(t *testing.T) {
	type args struct {
		count int
	}
	tests := []struct {
	...
	}{
		{
			notes:      Group(1, 2),
			args:       args{3},
			wantResult: Group(1, 2, 1, 2, 1, 2),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if gotResult := tt.notes.Repeat(tt.args.count); !reflect.DeepEqual(gotResult, tt.wantResult) {
				t.Errorf("NoteGroup.Repeat() = %v, want %v", gotResult, tt.wantResult)
			}
		})
	}
}
